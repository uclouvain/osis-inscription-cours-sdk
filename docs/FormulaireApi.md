...
------------- | ------------- | -------------
[**enregistrer_formulaire_proposition_pae**](FormulaireApi.md#enregistrer_formulaire_proposition_pae) | **POST** /{code_programme}/formulaire/ | 
...
> enregistrer_formulaire_proposition_pae(code_programme, formulaire_proposition_programme_annuel)



Save the current state of the enrollment form
...
with osis_inscription_cours_sdk.ApiClient(configuration) as api_client:
    ...
    formulaire_proposition_programme_annuel = FormulairePropositionProgrammeAnnuel(
        inscriptions_dans_tronc_commun=CodesUnitesEnseignement([
            "LINFO1341",
        ]),
        inscriptions_dans_mini_formations=[
            FormulairePropositionProgrammeAnnuelInscriptionsDansMiniFormations(
                ...
                codes_unites_enseignement=CodesUnitesEnseignement([
                    "LINFO1341",
                ]),
            ),
        ],
        demande_particuliere="J'aimerais choisir ce cours parce que ...",
    ...
    try:
        api_instance.enregistrer_formulaire_proposition_pae(code_programme, formulaire_proposition_programme_annuel)
    except osis_inscription_cours_sdk.ApiException as e:
        print("Exception when calling FormulaireApi->enregistrer_formulaire_proposition_pae: %s\n" % e)

    # example passing only required values which don't have defaults set
    # and optional values
    try:
        api_instance.enregistrer_formulaire_proposition_pae(code_programme, formulaire_proposition_programme_annuel, accept_language=accept_language, x_user_first_name=x_user_first_name, x_user_last_name=x_user_last_name, x_user_email=x_user_email, x_user_global_id=x_user_global_id)
    except osis_inscription_cours_sdk.ApiException as e:
        print("Exception when calling FormulaireApi->enregistrer_formulaire_proposition_pae: %s\n" % e)
```
 ...
 **formulaire_proposition_programme_annuel** | [**FormulairePropositionProgrammeAnnuel**](FormulairePropositionProgrammeAnnuel.md)|  |
